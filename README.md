...
5. **Saves to memory** – Creates new entries in database  
...
Create tables in Supabase using:
...
Known Limitations
Tool Loop with Ollama – The free Ollama model may not always follow the tool call format perfectly. Claude API would perform better but requires paid subscription.
Image Generation – Currently uses placeholder images. For production, integrate Replicate API or DALL·E with proper API keys.
Demo Mode – Runs without authentication. For production, implement proper auth.
License
